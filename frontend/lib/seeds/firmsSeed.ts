...
import type { Firm } from "@/lib/services/firmService";

const FIRMS_COLLECTION = "firms";

const SEED_DATA: Record<string, Omit<Firm, "createdAt" | "updatedAt">> = {
  "27ABCDE1234F1Z0": {
    ...
    history: [
      ...
      },
      {
        ...
        location: { lat: 19.0825, lng: 72.8830 },
        priorities: {
          monthly: [
...
export async function seedFirms(): Promise<void> {
  try {
    ...
    console.log(`✅ Successfully seeded ${seededCount} firms to Firestore`);
  } catch (error) {
